...
evaluate_feature_reduction_svm <- function(train,test,to_analyze_cols){
    svmmodel<- SupportVectorMachine
    corr_matrix <- cor(subset(train,select=to_analyze_cols) )
    projections_coefficients <- perform_pca(train,corr_matrix,2,to_analyze_cols,TRUE)
    train_projections <- project(train[to_analyze_cols],projections_coefficients)
    svmmodel <- train(svmmodel, cbind(train_projections,train["Class"]))
    test_projections <-project(test[to_analyze_cols],projections_coefficients)
    svmmodel <- test(svmmodel, cbind(test_projections,test["Class"]))
    return(svmmodel)
}
...
names(datasets) <- c("Mean","Std","Energy")
